...
const connectDB = async () => {
  try {
    ...
    console.log('Please make sure MongoDB is running and MONGODB_URL is set in .env file');
    console.log('Try running: mongod --version');
    process.exit(1);
  }
};
  ...
  try {
    ...
    for (let i = 0; i < sampleCourses.length; i++) {
      ...
      const course = await Course.create({
        ...
        studentsEnrolled: [],
        sold: Math.floor(Math.random() * 50) + 10, // Random sales between 10-60
      });
